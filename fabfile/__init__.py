...
def public_app(port='8001'):
    ...
    """
    if env.get('settings'):
        local("DEPLOYMENT_TARGET=%s bash -c 'gunicorn -b 0.0.0.0:%s --timeout 3600 --reload --log-file=logs/public_app.log public_app:wsgi_app'" % (env.settings, port))
    else:
        local('concurrently "%s" "npm start"' % gunicorn)
...
def deploy(remote='origin', reload=False):
    ...
    if app_config.DEPLOY_TO_SERVERS:
        ...
        servers.fabcast('servers.install_requirements')
